import string
from collections import defaultdict
from functools import reduce
from typing import Counter

TARGET = "CTF is for N00B"

confusion = []
diffusion = [
    0xf26cb481, 0x16a5dc92, 0x3c5ba924, 0x79b65248, 0x2fc64b18, 0x615acd29, 0xc3b59a42, 0x976b2584,
    0x6cf281b4, 0xa51692dc, 0x5b3c24a9, 0xb6794852, 0xc62f184b, 0x5a6129cd, 0xb5c3429a, 0x6b978425,
    0xb481f26c, 0xdc9216a5, 0xa9243c5b, 0x524879b6, 0x4b182fc6, 0xcd29615a, 0x9a42c3b5, 0x2584976b,
    0x81b46cf2, 0x92dca516, 0x24a95b3c, 0x4852b679, 0x184bc62f, 0x29cd5a61, 0x429ab5c3, 0x84256b97]


def make_conf(inp, conf):
    return [conf[x] for x in inp]


def inside_forward(out, mask):
    return [reduce(lambda x, y: x ^ y, [out[k]*((m >> k) & 1) for k in range(32)]) for m in mask]


def firstforward(out, conf, mask, n=256):
    for i in range(n):
        out = make_conf(out, conf)
        out = inside_forward(out, mask)
    return out


def lastforward(inp, conf):
    return [conf[inp[i*2]] ^ conf[inp[i*2+1]+256] for i in range(len(inp)//2)]


def main_me(inp=input, conf=confusion, mask=diffusion):
    return lastforward(firstforward(inp, conf, mask), conf)

def check_input(input):
    input = "".join([x for x in input if x in string.hexdigits+",[]xX"])
    try:
        input = eval(input)
    except Exception as e:
        raise Exception("L'argument n'est pas une liste d'entiers ou d'hexa (du type [0xA0, 10, 1, ...])")
    if len(input) != 32:
        raise Exception("Une liste de longueur 32 est demandée")
    if sum([type(x) != int for x in input]) != 0:
        raise Exception("La liste doit être composée d'entiers ou d'hexa !")
    return input

def checkflag(input):

    input = check_input(input)
    target = [ord(x) for x in TARGET]+[0]
    output = main_me(input, confusion, diffusion)
    if sum(x==y for x,y in zip(target,output))!=16:
        rep = "".join(chr(x) for x in output)
        raise Exception(f"Cible non atteinte : {rep} != {TARGET}")
    return "FLAG={INDEMPOTENTPOWA}"


